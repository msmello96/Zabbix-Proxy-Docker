...
ARG BUILD_BASE_IMAGE=zabbix/zabbix-build-sqlite3:ol-${ZBX_VERSION}
...
RUN microdnf -y install --enablerepo="ol9_developer_EPEL" yum freetds-devel vim net-tools gcc pkgconfig make libpq-devel glibc libpq postgresql-odbc unzip telnet tzdata curl mariadb-connector-odbc.x86_64 wget python3 python3-requests &&         rm -rf /etc/localtime &&     ln -s /usr/share/zoneinfo/America/Sao_Paulo /etc/localtime

COPY odbcinst.ini /etc/odbcinst.ini

RUN yum install -y https://download.oracle.com/otn_software/linux/instantclient/191000/oracle-instantclient19.10-basic-19.10.0.0.0-1.x86_64.rpm

RUN yum install -y https://download.oracle.com/otn_software/linux/instantclient/191000/oracle-instantclient19.10-odbc-19.10.0.0.0-1.x86_64.rpm

RUN microdnf -y clean all && rm -rf /var/cache/yum /var/lib/yum/yumdb/* /usr/lib/udev/hwdb.d/* && rm -rf /var/cache/dnf
